...
- **Summary**  
  사용자가 소셜 계정을 이용하여 아이루트 시스템에 간편하게 로그인한다. 소셜 로그인에 성공하면 사용자는 별도의 일반 로그인 절차 없이 아이루트 서비스를 이용할 수 있다.
- **Scope**
  아이루트
- **Level**   
  Guest level
...
- **Failed Post Condition**
  1. 소셜 인증이 실패하거나 중단되면 로그인은 완료되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 아이루트 로그인 화면에 접속한다.                    |
| 1    | 이 Use case는 사용자가 소셜 로그인을 시도할 때 시작된다.          |
| 2    | 사용자는 “소셜 로그인” 버튼을 클릭한다.           |
| 3    | 아이루트 시스템은 소셜 인증 페이지 또는 앱으로 이동시킨다.               |
| 4    | 사용자는 소셜 계정을 이용해 인증을 진행한다.                |
| 5    | 소셜 인증이 완료되면 아이루트 시스템은 사용자 식별 정보를 전달받는다.                |
| 6    | 아이루트 시스템은 기존 가입 여부를 확인한다.                |
| 7    | 기존 가입 사용자이면 로그인을 처리한다.                |
| 8    | 신규 사용자이면 추가 정보 입력 또는 사용자 유형 선택 화면으로 이동시킨다.                |
| 9    | 이 Use case는 사용자가 성공적으로 인증되면 끝난다.                |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 4a   | 사용자가 인증 절차를 중단하는 경우 사용자가 인증 절차를 중단한다.              |
| 5a   | 인증 서버와의 통신에 실패하는 경우 “소셜 로그인에 실패했습니다. 잠시 후 다시 시도해 주세요.”라고 안내한다.              |
| 6a   | 소셜 계정과 기존 회원 정보가 충돌하는 경우  추가 본인 확인 또는 계정 연동 절차를 요구한다.              |
#### RELATED INFORMATION
- **Performance**:
  3초 이내에 완료되어야 한다.
- **Frequency**:
  회원당 하루에 여러 번 수행 가능
- **Concurrency**:
  동일 소셜 계정으로 동시에 여러 로그인 요청이 들어온 경우 가장 최근 인증 결과를 우선 적용한다.
- **Due Date**:
  ...
...
GENERAL CHARACTERISTICS
- **Summary**  
  등록된 사용자가 아이디와 비밀번호를 입력하여 아이루트 시스템에 로그인한다.
- **Scope**
  아이루트
- **Level**   
  Guest level
...
| ---- | --------------------------------------------------------- |
| S    | 사용자가 아이루트 로그인 화면에 접속한다.                    |
| 1    | 이 Use case는 사용자가 일반 로그인을 시도할 때 시작된다.          |
| 2    | 사용자는 아이디와 비밀번호를 입력한다.         |
| 3    | 사용자는 “로그인” 버튼을 클릭한다.          |
| 4    | 아이루트 시스템은 입력된 인증 정보를 검증한다.                |
| 5    | 아이루트 시스템은 사용자 계정을 확인하고 인증 토큰을 발급한다.                |
| 6    | 아이루트 시스템은 사용자 유형에 맞는 메인 화면으로 이동시킨다.               |
| 7    | 이 Use case는 로그인에 성공하면 끝난다.                |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 4a   | 아이디 또는 비밀번호가 올바르지 않을 경우 “아이디 또는 비밀번호가 올바르지 않습니다.”라는 메시지를 보여준다.             |
| 4b  | 로그인 실패가 일정 횟수 이상 누적되면 일정 시간 동안 추가 로그인을 제한한다.	|
| 5a   | 계정 상태가 비활성 또는 탈퇴 처리 상태이면 로그인 실패 메시지를 보여준다.         |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
...
GENERAL CHARACTERISTICS
- **Summary**  
  비회원 사용자가 아이루트 시스템을 이용하기 위해 기본 회원 정보를 입력하여 신규 계정을 등록한다.
- **Scope**
  아이루트
- **Level**   
  Guest level
...
- **Success Post Condition**
    ...
    2. 사용자는 이후 로그인할 수 있다.
- **Failed Post Condition**
  1. 필수 정보가 누락되거나 중복 또는 유효하지 않은 경우 회원가입은 완료되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 아이루트 회원가입 화면에 접속한다.                    |
| 1    | 이 Use case는 사용자가 회원가입을 시도할 때 시작된다.        |
| 2    | 사용자는 아이디, 비밀번호, 이름, 이메일, 전화번호 등의 필수 정보를 입력한다.          |
| 3    | 사용자는 개인정보 수집 및 이용 동의 내용을 확인하고 동의한다.               |
| 4    | 사용자는 “회원가입” 버튼을 클릭한다.               |
| 5    | 아이루트 시스템은 입력값의 유효성과 중복 여부를 검증한다.              |
| 6    | 아이루트 시스템은 새로운 계정을 생성한다.               |
| 7    | 아이루트 시스템은 “회원가입이 완료되었습니다.”라는 메시지를 보여준다.            |
| 8    | 이 Use case는 계정 생성이 완료되면 끝난다.           |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 필수 정보가 누락되면 누락된 항목을 표시한다.              |
| 5a   | 아이디 또는 이메일이 이미 사용 중이면 중복 메시지를 보여준다.            |
| 5b   | 비밀번호가 정책에 맞지 않으면 비밀번호 규칙을 안내한다.            |
| 6a   | 서버 오류로 계정 생성에 실패하면 "회원가입에 실패했습니다. 잠시 후 다시 시도해 주세요" 라고 안내한다. |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
  개별 사용자 기준 일반적으로 1회 수행
- **Concurrency**:
  제한 없음
- **Due Date**:
  ...
...
- **Success Post Condition**
  1. 사용자는 자신의 프로필 정보를 확인할 수 있다.
- **Failed Post Condition**
  1. 프로필 정보를 불러오지 못하면 화면에 정보가 표시되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 회원 프로필 정보 조회를 시도한다.                    |
| 1    | 사용자는 마이페이지 또는 프로필 메뉴를 선택한다.          |
| 2    | 아이루트 시스템은 저장된 프로필 정보를 조회한다.           |
| 3    | 아이루트 시스템은 사용자에게 이름, 연락처, 이메일, 사용자 유형 등의 정보를 보여준다.              |
| 4    | 이 Use case는 프로필 정보가 정상적으로 표시되면 끝난다.             |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 서버 오류 또는 네트워크 오류로 인해 정보를 불러오지 못한 경우 “프로필 정보를 불러올 수 없습니다. 다시 시도해 주세요.”라고 안내한다.         |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
...
- **Success Post Condition**
  1. 변경된 프로필 정보가 정상적으로 저장된다.
- **Failed Post Condition**
  1. 입력값이 유효하지 않거나 서버 오류가 발생하면 수정이 완료되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 이 Use case는 사용자가 내 프로필 수정을 시도할 때 시작된다.                 |
| 1    | 사용자가 로그인된 상태에서 프로필 수정 화면에 접근한다.         |
| 2    | 아이루트 시스템은 현재 저장된 프로필 정보를 보여준다.           |
| 3    | 사용자는 수정 가능한 항목을 변경한다.            |
| 4    | 사용자는 “저장” 버튼을 클릭한다.         |
| 5    | 아이루트 시스템은 입력값의 형식과 유효성을 검증한다.         |
| 6    | 아이루트 시스템은 변경된 정보를 저장한다.       |
| 7    | 아이루트 시스템은 “프로필이 수정되었습니다.”라는 메시지를 보여준다.       |
| 8    | 이 Use case는 프로필 수정이 완료되면 끝난다.         |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 3a   | 사용자가 잘못된 형식의 연락처 또는 이메일을 입력하면 오류 메시지를 보여준다.         |
| 6a   | 서버 오류로 저장에 실패하면 "프로필 수정에 실패했습니다. 다시 시도해 주세요" 라고 안내한다.         |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
  회원당 일주일에 한 번 정도 수행
- **Concurrency**:
  동일 계정에서 동시에 수정 요청이 발생하는 경우 최신 저장 요청을 우선 적용한다.
- **Due Date**:
  ...

### **Use case #6: 소프트 삭제(계정 탈퇴)
GENERAL CHARACTERISTICS
- **Summary**  
  로그인된 사용자가 계정 탈퇴를 요청하면, 아이루트 시스템은 계정을 즉시 완전 삭제하지 않고 소프트 삭제 상태로 전환한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Failed Post Condition**
  1. 시스템 오류가 발생하면 탈퇴 처리가 완료되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 이 Use case는 사용자가 계정 탈퇴를 시도할 때 시작된다.           |
| 1    | 사용자가 로그인된 상태에서 계정 탈퇴 메뉴에 접근한다.          |
| 2    | 사용자는 탈퇴 안내 사항을 확인한다.         |
| 3    | 사용자는 탈퇴 의사를 최종 확인한다.            |
| 4    | 아이루트 시스템은 계정 상태를 소프트 삭제 상태로 변경한다.             |
| 5    | 아이루트 시스템은 인증 정보를 무효화한다.            |
| 6    | 아이루트 시스템은 “회원 탈퇴가 완료되었습니다.”라는 메시지를 보여주고 초기 화면으로 이동시킨다.      |
| 7    | 이 Use case는 계정 탈퇴가 완료되면 끝난다.             |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 3a   | 사용자가 최종 확인 단계에서 취소하면 탈퇴 절차를 중단하고 이전 화면으로 복귀시킨다.    |
| 4a   | 서버 오류로 상태 연결에 실패하면 "회원 탈퇴 처리에 실패했습니다. 다시 시도해 주세요" 라고 안내한다.  |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
  개별 사용자 기준 매우 낮음
- **Concurrency**:
  동일 계정에 대해 중복 탈퇴 요청이 들어온 경우 최초 처리 요청만 유효하게 반영한다.
- **Due Date**:
  ...
...
GENERAL CHARACTERISTICS
- **Summary**  
  로그인된 사용자가 아이루트 시스템에서 안전하게 로그아웃하여 인증 상태를 종료한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Failed Post Condition**
  1. 로그아웃 처리 실패 시 사용자가 로그인 상태로 남아 있을 수 있다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 이 Use case는 로그인한 사용자가 로그아웃을 시도할 때 시작된다.                  |
| 1    | 사용자는 “로그아웃” 버튼을 클릭한다.          |
| 2    | 아이루트 시스템은 현재 인증 토큰 또는 세션을 무효화한다.           |
| 3    | 아이루트 시스템은 “로그아웃되었습니다.”라는 메시지를 보여준다.             |
| 4    | 아이루트 시스템은 로그인 화면 또는 초기 화면으로 이동시킨다.             |
| 5    | 이 Use case는 로그아웃이 완료되면 끝난다.            |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 서버 오류 또는 네트워크 오류로 인해 로그아웃 처리에 실패한 경우 “로그아웃에 실패했습니다. 다시 시도해 주세요.”라고 안내한다.      |
#### RELATED INFORMATION
- **Performance**:
  1초 이내에 완료되어야 한다.
- **Frequency**:
  회원당 하루에 여러 번 수행 가능
- **Concurrency**:
  동일 계정이 여러 기기에서 로그인된 경우 현재 기기만 로그아웃하거나 전체 로그아웃 정책을 적용할 수 있다.
- **Due Date**:
  ...
...
GENERAL CHARACTERISTICS
- **Summary**  
  로그인된 사용자의 액세스 토큰이 만료되었을 때, 아이루트 시스템은 유효한 리프레시 토큰을 이용해 새로운 액세스 토큰을 발급한다.
- **Scope**
  아이루트
- **Level**   
...
- **Failed Post Condition**
  1. 리프레시 토큰이 유효하지 않으면 토큰 재발급에 실패하고 재로그인이 필요하다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 이 Use case는 액세스 토큰 재발급이 필요할 때 시작된다.                  |
| 1    | 사용자가 로그인된 상태로 서비스를 이용 중이다.         |
| 2    | 아이루트 시스템은 액세스 토큰 만료를 감지한다.        |
| 3    | 아이루트 시스템은 저장된 리프레시 토큰의 유효성을 확인한다.             |
| 4    | 아이루트 시스템은 새로운 액세스 토큰을 발급한다.             |
| 5    | 아이루트 시스템은 사용자 요청을 계속 처리한다.          |
| 6    | 이 Use case는 토큰 재발급이 완료되면 끝난다. 	|
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 리프레시 토큰이 만료되었거나 유효하지 않으면 토큰 재발급을 중단하고 재로그인을 요구한다.    |
| 3a   | 서버 오류로 토큰 발급에 실패하면 요청 처리에 실패하고 오류를 반환한다.    |
#### RELATED INFORMATION
- **Performance**:
  1초 이내에 완료되어야 한다.
- **Frequency**:
  토큰 만료 주기에 따라 반복적으로 발생 가능
- **Concurrency**:
  동일 계정에서 동시에 여러 재발급 요청이 발생하면 가장 최근 요청 기준으로 처리한다.
- **Due Date**:
  ...
...
GENERAL CHARACTERISTICS
- **Summary**  
  로그인된 사용자가 현재 비밀번호를 확인한 뒤 새로운 비밀번호로 변경한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Failed Post Condition**
  1. 현재 비밀번호가 일치하지 않거나 새 비밀번호가 정책에 맞지 않으면 변경되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 이 Use case는 로그인한 사용자가 비밀번호 변경을 시도할 때 시작된다.                 |
| 1    | 사용자는 현재 비밀번호, 새 비밀번호, 새 비밀번호 확인 값을 입력한다.        |
| 2    | 사용자는 “변경” 버튼을 클릭한다.        |
| 3    | 아이루트 시스템은 현재 비밀번호 일치 여부를 검증한다.           |
| 4    | 아이루트 시스템은 새 비밀번호의 정책 충족 여부를 검증한다.             |
| 5    | 아이루트 시스템은 비밀번호를 새로운 값으로 저장한다.         |
| 6    | 아이루트 시스템은 “비밀번호가 변경되었습니다.”라는 메시지를 보여준다.	|
| 7    | 이 Use case는 비밀번호 변경이 완료되면 끝난다.	|
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 3a   | 현재 비밀번호가 일치하지 않으면 오류 메시지를 보여주고 다시 입력을 요구한다.   |
| 4a   | 새 비밀번호가 정책에 맞지 않으면 비밀번호 규칙을 안내한다.     |
| 5a   | 서버 오류로 저장에 실패하면 “비밀번호 변경에 실패했습니다. 다시 시도해 주세요.”라고 안내한다.     |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
  회원당 필요 시 수시 수행 가능
- **Concurrency**:
  동일 계정에서 비밀번호 변경 요청이 동시에 발생하는 경우 가장 마지막 성공 요청을 기준으로 적용한다.
- **Due Date**:
  ...
...
GENERAL CHARACTERISTICS
- **Summary**  
  사용자가 비밀번호를 분실했을 때 본인 확인 절차를 거쳐 새로운 비밀번호를 설정할 수 있도록 한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Success Post Condition**
  1. 사용자는 본인 확인 후 새로운 비밀번호를 설정할 수 있다. 수 있다.
- **Failed Post Condition**
  1. 본인 확인에 실패하면 비밀번호 재설정은 진행되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 이 Use case는 사용자가 비밀번호 찾기를 시도할 때 시작된다.                 |
| 1    | 사용자는 “비밀번호 찾기” 버튼을 클릭한다.        |
| 2    | 아이루트 시스템은 아이디와 이메일 또는 전화번호를 입력받는다.        |
| 3    | 사용자는 계정 식별 정보를 입력한다.          |
| 4    | 아이루트 시스템은 가입 정보를 대조한다.            |
| 5    | 아이루트 시스템은 인증 코드 또는 인증 링크를 전송한다.         |
| 6    | 사용자는 인증 절차를 완료한다.	|
| 7    | 아이루트 시스템은 새 비밀번호 설정 화면을 보여준다.	|
| 8    | 사용자는 새 비밀번호를 입력한다.	|
| 9    | 아이루트 시스템은 새 비밀번호를 저장한다.	|
| 10   | 아이루트 시스템은 “비밀번호가 재설정되었습니다.”라는 메시지를 보여준다. 	|
| 11   | 이 Use case는 비밀번호 찾기가 완료되면 끝난다.	|
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 4a   | 입력한 정보와 일치하는 계정이 없으면 계정을 찾을 수 없다고 안내한다.   |
| 5a   | 인증 정보 전송에 실패하면 잠시 후 다시 시도해 달라고 안내한다.     |
| 6a   | 인증 코드 입력이 여러 차례 실패한다면 일정 시간 동안 재시도를 제한한다.    |
| 9a   | 서버 오류로 저장에 실패하면 재설정 실패 메시지를 보여준다.    |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
  빈도가 낮은 편
- **Concurrency**:
  동일 계정에서 동시에 여러 재설정 요청이 들어온 경우 가장 최근 요청만 유효하다.
- **Due Date**:
  ...

### **Use case #11: 사용자 유형 선택
GENERAL CHARACTERISTICS
- **Summary**  
  사용자가 회원가입 또는 최초 로그인 이후 자신의 사용자 유형을 선택한다. 사용자 유형은 학부모, 학원, 관리자 중 하나이다.
- **Scope**
  아이루트
- **Level**   
  Guest level
...
- **Primary Actor**
  Guest
- **Preconditions**
  1. 사용자는 회원가입을 완료했거나 최초 로그인을 마친 상태여야 한다.
- **Trigger**
...
- **Failed Post Condition**
  1. 유형 선택이 완료되지 않으면 일부 서비스 이용이 제한될 수 있다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 이 Use case는 사용자가 사용자 유형 선택을 진행할 때 시작된다.                |
| 1    | 아이루트 시스템은 학부모, 학원, 관리자 유형 선택 화면을 보여준다.        |
| 2    | 사용자는 자신의 유형을 하나 선택한다.       |
| 3    | 사용자는 “확인” 버튼을 클릭한다.          |
| 4    | 아이루트 시스템은 선택한 유형 정보를 사용자 계정에 저장한다.            |
| 5    | 아이루트 시스템은 선택된 유형에 맞는 메인 화면 또는 추가 정보 입력 화면으로 이동시킨다.         |
| 6    | 이 Use case는 사용자 유형 선택이 완료되면 끝난다.	|
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 사용자가 유형을 선택하지 않고 진행하려고 한다면 유형 선택이 필수임을 안내한다.   |
| 4a   | 서버 오류로 저장에 실패하면 “사용자 유형 저장에 실패했습니다. 다시 시도해 주세요.”라고 안내한다.  |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
  회원당 최초 설정 시 1회 수행
- **Concurrency**:
  동일 계정에 대해 동시에 여러 유형 선택 요청이 들어오면 마지막 성공 요청을 반영한다.
- **Due Date**:
  ...
...
GENERAL CHARACTERISTICS
- **Summary**  
  자유 게시판, 공지 사항, 문의 게시판, 일반 게시판(학원, 학부모)을 조회 할 수 있다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Success Post Condition**
    1. 게시판 목록(자유, 공지, 문의는 상단에 고정하고 나머지 게시판은 최신순으로 나열)이 화면에 정상적으로 표시된다.
- **Failed Post Condition**
    1. 서버 통신 실패 시 게시판 목록이 표시되지 않고 오류 메시지가 출력된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 게시판 내용을 확인하고자 한다.        |
| 1    | 사용자는 메인 화면에서 ‘게시판’ 버튼을 클릭한다.                 |
| 2    | 클라이언트는 서버에 게시판 목록 데이터를 요청한다.                 |
| 3    | 서버는 게시판 목록을 반환한다.                 |
| 4    | 클라이언트는 수신한 데이터를 기반으로 게시판 목록 UI를 화면에 표시한다.  |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 3a   | 서버에 게시판 데이터가 존재하지 않는 경우 빈 목록을 반환한다.            |
| 4a   | 클라이언트는 “등록된 게시판이 없습니다” 메시지를 표시한다.                 |
#### RELATED INFORMATION
- **Performance**:
  게시판 목록 조회 및 화면 렌더링은 2초 이내 완료되어야 한다.
- **Frequency**:
  사용자당 앱 접속 시 매일 하루 평균 1회 확인된다.
- **Concurrency**:
  다수의 사용자가 동시에 정보를 조회하더라도 처리할 수 있어야 하며, 최대 1,000건 이상의 동시 조회 요청을 처리할 수 있도록 설계한다.
- **Due Date**:
  2026.03.31

### **Use case #45 : 새로운 게시판/게시글 작성
GENERAL CHARACTERISTICS
- **Summary**  
  새로운 게시판 또는 게시글을 생성할 수 있다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Preconditions**
    1. 사용자는 로그인 상태여야 한다.
- **Trigger**
  사용자가 게시판/게시글 생성 버튼을 클릭한다.
- **Success Post Condition**
    1. 새로운 게시판/게시글이 생성되고 목록에 반영된다.
- **Failed Post Condition**
    1. 생성에 실패하며 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 새로운 게시판/게시글을 생성하고자 한다.       |
| 1    | 사용자는 화면에서 '새 게시판 생성', '새 게시글 작성' 버튼을 클릭한다. |
| 2    | 클라이언트는 게시판 생성에서는 게시판 이름, 간단한 소개글, 위치, 태그 입력 폼, 게시글 작성에서는 게시글 제목, 소개글, 사진 첨부, 익명 여부 등의 입력 폼을 사용자에게 제공한다.                |
| 3    | 사용자는 정보를 입력하고 생성 요청을 수행한다.                 |
| 4    | 클라이언트는 서버에 생성 요청을 전송한다.                 |
| 5    | 서버는 입력 데이터를 검증한다.                 |
| 6    | 서버는 게시판을 삭제하고 DB에 반영한다.                 |
| 7    | 서버는 삭제 완료 응답을 클라이언트에 반환한다.                 |
| 8    | 클라이언트는 목록을 갱신한 데이터를 기반으로 게시판 목록 UI를 화면에 표시한다.              |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 3a   | 게시판 이름이 비어 있거나 유효하지 않은 경우 클라이언트는 “올바른 게시판 이름을 입력하세요” 메시지를 표시한다.                |
#### RELATED INFORMATION
- **Performance**:
  생성 요청 및 처리 시간은 2초 이내 완료되어야 한다.
- **Frequency**:
  게시판 생성 기능은 낮은 빈도로 사용된다.
- **Concurrency**:
  다수 사용자가 동시에 게시판을 생성하더라도 데이터 충돌이 발생하지 않도록 처리해야 한다.
- **Due Date**:
  2026.03.31

### **Use case #46 : 게시판/게시글 수정
GENERAL CHARACTERISTICS
- **Summary**  
  작성한 게시판/게시글을 수정할 수 있다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Status**
  Design
- **Primary Actor**
    1. 학부모 (Parent User), 학원 강사 (Instructor), 관리자 (Administrator)
- **Preconditions**
    1. 사용자는 로그인 상태여야 한다.
...
- **Success Post Condition**
    1. 게시판/게시글 정보가 수정되고 변경 사항이 게시판 목록에 반영된다.
- **Failed Post Condition**
    1. 수정에 실패하며 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 게시판/게시글 정보를 수정하고자 한다.                     |
| 1    | 클라이언트는 기존에 입력된 폼을 사용자에게 제공한다.    |
| 2    | 사용자는 수정할 정보를 입력하고 저장을 요청한다.                   |
| 3    | 클라이언트는 서버에 게시판 수정 요청을 전송한다.                |
| 4    | 서버는 입력 데이터를 검증한다.                          |
| 5    | 서버는 게시판 정보를 수정하고 DB에 반영한다.                 |
| 6    | 서버는 수정된 게시판 정보를 클라이언트에 반환한다.               |
| 7    | 클라이언트는 변경된 내용을 화면에 반영한다.(게시판 수정 시 게시판 목록 화면으로, 게시글 수정시 게시글 상세 조회 화면을 반영한다.)                   |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 3a   | 게시판 이름이 비어 있거나 유효하지 않은 경우 클라이언트는 “올바른 게시판 이름을 입력하세요” 메시지를 표시한다.                |
#### RELATED INFORMATION
- **Performance**:
  수정 요청 및 처리 시간은 2초 이내 완료되어야 한다.
- **Frequency**:
  게시판 수정 기능은 낮은 빈도로 사용된다.
- **Concurrency**:
  다수 사용자가 동시에 게시판을 수정하더라도 데이터 충돌이 발생하지 않도록 처리해야 한다.
- **Due Date**:
  2026.03.31

### **Use case #47 : 게시판/게시글 삭제
GENERAL CHARACTERISTICS
- **Summary**  
  작성한 게시판/게시글을 삭제할 수 있다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Success Post Condition**
    1. 게시판/게시글 정보가 삭제되고 변경 사항이 게시판 목록에 반영된다.
- **Failed Post Condition**
    1. 게시판 삭제에 실패하며 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 게시판/게시글 정보를 삭제하고자 한다.                     |
| 1    | 사용자는 게시판/게시글 설정 메뉴에서 ‘삭제’ 버튼을 클릭한다.            |
| 2    | 클라이언트는 삭제 여부 확인 팝업을 사용자에게 표시한다.           |
| 3    | 사용자는 삭제를 확정한다.               |
| 4    | 클라이언트는 서버에 게시판 삭제 요청을 전송한다.                |
| 5    | 서버는 게시판 정보를 삭제하고 DB에 반영한다.                 |
| 6    | 서버는 삭제 완료 응답을 클라이언트에 반환한다.               |
| 7    | 클라이언트는 게시판 목록을 갱신하여 삭제된 게시판을 제거한다.                |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 3a   | 삭제 확인 단계에서 사용자가 취소한 경우 삭제 요청이 수행되지 않는다.                |
#### RELATED INFORMATION
- **Performance**:
  삭제 요청 및 처리 시간은 2초 이내 완료되어야 한다.
- **Frequency**:
  게시판 삭제 기능은 매우 낮은 빈도로 사용된다.
- **Concurrency**:
  제한 없음
- **Due Date**:
  2026.03.31

### **Use case #48 : 게시판/게시글 검색
GENERAL CHARACTERISTICS
- **Summary**  
  제목, 소개글, 태그, 작성자 등을 기준으로 원하는 게시판을 검색할 수 있다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Success Post Condition**
    1. 검색 조건에 맞는 게시글/게시판 목록이 화면에 표시된다.
- **Failed Post Condition**
    1. 검색 요청 처리에 실패하면 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 특정 게시판/게시글을 찾고자 한다.                     |
| 1    | 사용자는 검색창에서 원하는 태그, 검색어를 입력한다.            |
| 2    | 사용자는 검색 버튼을 클릭하거나 엔터를 입력한다.           |
| 3    | 클라이언트는 서버에 검색 요청을 전송한다.               |
| 4    | 서버는 입력된 정보와 일치하는 데이터를 조회한다.                |
| 5    | 서버는 검색 결과를 클라이언트에 반환한다.                 |
| 6    | 클라이언트는 검색 결과에 해당하는 목록을 화면에 표시한다.               |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 사용자가 태그를 입력하거나 선택하지 않은 경우 클라이언트는 “태그를 선택 또는 입력하세요” 메시지를 표시한다.                |
| 4a   | 검색 결과와 일치하는 게시판이 없는 경우 서버는 빈 결과를 반환한다.         |
#### RELATED INFORMATION
- **Performance**:
  검색 요청 및 결과 조회는 2초 이내에 완료되어야 한다.
- **Frequency**:
  사용자가 특정 게시글/게시판을 검색할 때마다 수행되며, 비교적 자주 사용될 수 있다.
- **Concurrency**:
  제한 없음
- **Due Date**:
  2026.03.31
...
GENERAL CHARACTERISTICS
- **Summary**  
  자주 이용하는 게시판을 즐겨찾기로 등록하거나 해제할 수 있다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Preconditions**
    1. 사용자는 로그인 상태여야 한다.
- **Trigger**
  사용자가 게시판 화면에서 즐겨찾기 버튼을 클릭한다.
- **Success Post Condition**
    1. 해당 게시판이 즐겨찾기 목록에 추가되거나 제거된다.
- **Failed Post Condition**
    1. 요청 처리에 실패하면 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 게시판을 즐겨찾기에 추가하거나 해제하고자 한다.                     |
| 1    | 사용자는 게시판 목록 또는 상세 화면에서 즐겨찾기 버튼을 클릭한다.            |
| 2    | 클라이언트는 서버에 즐겨찾기 추가 또는 해제 요청을 전송한다.           |
| 3    | 서버는 즐겨찾기 상태를 변경하고 DB에 반영한다.               |
| 4    | 서버는 변경된 상태를 클라이언트에 반환한다.                |
| 5    | 클라이언트는 즐겨찾기 상태를 UI에 반영한다.                 |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 네트워크 오류 발생 시 클라이언트는 “요청 처리에 실패했습니다” 메시지를 표시한다.                |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
...
### **Use case #50 : 게시판 상세 보기(게시글 목록 조회)
GENERAL CHARACTERISTICS
- **Summary**  
  특정 게시판에 접속하여 해당 게시판의 게시글 목록을 (최신순, 조회순, 공감순, 댓글순 등)조회할 수 있다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Trigger**
  사용자가 특정 게시판을 클릭한다.
- **Success Post Condition**
    1. 해당 게시판의 게시글 목록이 화면에 표시된다.
- **Failed Post Condition**
    1. 게시글 목록 조회 실패하면 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 게시판의 게시글 목록을 확인하고자 한다.                     |
| 1    | 사용자는 게시판 목록에서 특정 게시판을 클릭한다.            |
| 2    | 클라이언트는 서버에 게시글 목록 조회 요청을 전송한다.           |
| 3    | 서버는 해당 게시판의 게시글 데이터를 조회한다.               |
| 4    | 서버는 게시글 목록을 클라이언트에 반환한다.                |
| 5    | 클라이언트는 게시글 목록을 화면에 표시한다.                 |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 네트 워크 오류 발생 시 클라이언트는 “요청 처리에 실패했습니다” 메시지를 표시한다.                |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
  사용자당 앱 접속 시 매일 하루 평균 1회 확인된다.
- **Concurrency**:
  다수의 사람이 동시에 입장해도 처리가 가능해야 한다.
- **Due Date**:
  2026.03.31

### **Use case #51 : 게시글 상세 보기
GENERAL CHARACTERISTICS
- **Summary**  
  특정 게시글을 선택하여 상세 내용을 확인할 수 있다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Trigger**
  사용자가 게시글을 클릭한다.
- **Success Post Condition**
    1. 게시글의 상세 내용이 화면에 표시된다.
- **Failed Post Condition**
    1. 게시글 조회 실패하면 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 게시글 내용을 확인하고자 한다.                     |
| 1    | 사용자는 게시글 목록에서 특정 게시글을 클릭한다.(조회수 상승)           |
| 2    | 클라이언트는 서버에 게시글 상세 조회 요청을 전송한다.           |
| 3    | 서버는 게시글 데이터를 조회한다.               |
| 4    | 서버는 게시글 정보를 클라이언트에 반환한다.                |
| 5    | 클라이언트는 게시글 내용을 화면에 표시한다.                 |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 네트 워크 오류 발생 시 클라이언트는 “요청 처리에 실패했습니다” 메시지를 표시한다.                |
...
### **Use case #52 : 공감 또는 댓글 달기
GENERAL CHARACTERISTICS
- **Summary**  
  게시글에 공감을 표시하거나 댓글을 작성할 수 있다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Preconditions**
    1. 사용자는 로그인 상태여야 한다.
- **Trigger**
  사용자가 공감 버튼 또는 댓글 작성 기능을 이용한다.
- **Success Post Condition**
    1. 공감 수가 증가하거나 댓글이 등록된다.
- **Failed Post Condition**
    1. 게시글 조회 실패하면 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
| S    | 사용자가 게시글에 대한 자신의 생각을 입력하고자 한다.                     |
| 1    | 사용자는 공감 버튼을 클릭하거나 댓글을 입력한다.           |
| 2    | 클라이언트는 서버에 요청을 전송한다.           |
| 3    | 서버는 데이터를 저장하고 반영한다.               |
| 4    | 서버는 결과를 클라이언트에 반환한다.                |
| 5    | 클라이언트는 변경 사항을 화면에 반영한다.                 |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                                |
| ---- | --------------------------------------------------------- |
| 2a   | 네트 워크 오류 발생 시 클라이언트는 “요청 처리에 실패했습니다” 메시지를 표시한다.                |
...
GENERAL CHARACTERISTICS
- **Summary**  
  사용자가 특정 사용자와 메시지를 주고받기 위한 채팅방을 생성한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Preconditions**
    1. 사용자는 로그인 상태여야 한다.
    2. 채팅 상대가 시스템에 등록된 사용자여야 한다.
- **Trigger**
  사용자가 특정 사용자와의 채팅 시작 버튼을 클릭한 경우
- **Success Post Condition**
    1. 새로운 채팅방이 생성된다.
    2. 해당 채팅방이 채팅 목록에 추가된다.
- **Failed Post Condition**
    1. 채팅방 생성에 실패하면 채팅은 시작되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                       |
| ---- |------------------------------|
| S    | 사용자가 상대방 프로필 또는 목록 화면에 접속한다. |
| 1    | 사용자가 “채팅하기” 버튼을 클릭한다.        |
| 2    | 시스템은 기존 채팅방 존재 여부를 확인한다.     |
| 3    | 기존 채팅방이 없으면 새로운 채팅방을 생성한다.        |
| 4    | 생성된 채팅방 화면으로 이동한다.       |
#### EXTENSION SCENARIOS
| Step | Branching Action                                                              |
|------| ------------------------------------------------------- |
| 2a   | 기존 채팅방이 존재하는 경우 해당 채팅방으로 이동한다.              |
| 3a   | 채팅방 생성 중 오류 발생 시 오류 메시지를 출력한다.            |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
  중간 빈도로 사용된다.
- **Concurrency**:
  여러 사용자가 동시에 채팅방 생성을 요청하더라도 각 사용자 요청은 독립적으로 정상 처리되어야 한다.
- **Due Date**:
  ...

### **Use case #54 : 채팅방 목록 조회
GENERAL CHARACTERISTICS
- **Summary**  
  사용자가 자신이 참여 중인 채팅방 목록을 조회한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Trigger**
  사용자가 채팅 메뉴에 접속한 경우
- **Success Post Condition**
    1. 채팅방 목록이 최신 메시지 기준으로 정렬되어 표시된다.
- **Failed Post Condition**
    1. 채팅방 목록에 아무것도 표시되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                     |
| ---- |----------------------------|
| S    | 사용자가 채팅 메뉴에 접근한다.          |
| 1    | 시스템은 사용자의 채팅방 목록을 조회한다.     |
| 2    | 마지막 메시지 기준으로 정렬한다.   |
| 3    | 채팅방 목록을 화면에 표시한다. |
| 4    | 생성된 채팅방 화면으로 이동한다.         |
#### EXTENSION SCENARIOS
| Step | Branching Action                     |
|------|--------------------------------------|
| 2a   | 네트워크 오류나 기존 채팅방이 없을 시 채팅방이 없음을 표시하다. |
#### RELATED INFORMATION
- **Performance**:
  2초 이내에 완료되어야 한다.
- **Frequency**:
  높은 빈도로 사용된다.
- **Concurrency**:
  다수의 사용자가 동시에 채팅방 목록을 조회하더라도 각 사용자에게 최신 목록이 정상적으로 제공되어야 한다.
- **Due Date**:
  ...
### **Use case #55 : 채팅방 삭제
GENERAL CHARACTERISTICS
- **Summary**  
  사용자가 더 이상 필요 없는 채팅방을 삭제한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Preconditions**
    1. 사용자는 로그인 상태여야 한다.
    2. 사용자는 채팅방에 참여 중이어야 한다.
    3. 네트워크 연결이 정상이어야 한다.
- **Trigger**
  사용자가 채팅방 삭제 버튼을 클릭한 경우
- **Success Post Condition**
    1. 해당 채팅방이 채팅방 목록에서 제거된다.
- **Failed Post Condition**
    1. 채팅방 목록에서 제거 되지 않고 오류 메시지를 출력한다.
#### MAIN SUCCESS SCENARIO
| Step | Action                |
| ---- | --------------------- |
| S    | 사용자가 채팅방 목록 화면에 접속한다. |
| 1    | 삭제할 채팅방을 선택한다.        |
| 2    | “삭제” 버튼을 클릭한다.        |
| 3    | 시스템은 삭제 여부를 확인한다.     |
| 4    | 채팅방을 삭제한다.            |
...
- **Frequency**:
  중간 빈도로 사용된다.
- **Concurrency**:
  동일 채팅방에 대해 동시에 삭제 또는 접근 요청이 발생하더라도 데이터 정합성이 유지되어야 한다.
- **Due Date**:
  ...

### **Use case #56 : 텍스트 메시지 전송
GENERAL CHARACTERISTICS
- **Summary**  
  사용자가 채팅방에서 텍스트 메시지를 전송한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Success Post Condition**
    1. 메시지가 상대방에게 전달된다.
- **Failed Post Condition**
    1. 텍스트 문자는 저장되고 상대방에게 전달이 안되어 전송 실패와 재전송 문자를 표시한다.
#### MAIN SUCCESS SCENARIO
| Step | Action              |
| ---- | ------------------- |
| S    | 사용자가 채팅방에 접속한다.     |
| 1    | 메시지를 입력한다.          |
| 2    | 전송 버튼을 클릭한다.        |
| 3    | 시스템은 메시지를 서버로 전송한다. |
| 4    | 서버는 메시지를 저장한다.      |
| 5    | 상대방에게 메시지를 전달한다.    |
#### EXTENSION SCENARIOS
| Step | Branching Action              |
| ---- | ----------------------------- |
| 3a   | 네트워크 오류 발생 시 전송 실패 메시지를 표시한다. |
#### RELATED INFORMATION
- **Performance**:
  1초 이내에 완료되어야 한다.
- **Frequency**:
  매우 높은 빈도로 사용된다.
- **Concurrency**:
  여러 사용자가 동시에 메시지를 전송하더라도 메시지 누락이나 순서 오류 없이 처리되어야 한다.
- **Due Date**:
  ...
...
GENERAL CHARACTERISTICS
- **Summary**  
  사용자가 메시지를 확인하면 해당 메시지의 읽음 상태를 업데이트한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Preconditions**
    1. 사용자는 로그인 상태여야 한다.
    2. 사용자는 메시지를 수신한 상태여야 한다.
    3. 네트워크 연결이 정상이어야 한다.
- **Trigger**
  사용자가 채팅방에 들어가 메시지를 확인한 경우
- **Success Post Condition**
    1. 메시지의 읽음 상태가 “읽음”으로 변경된다.
- **Failed Post Condition**
    1. 메시지의 읽음 상태가 변경되지 않는다.
#### MAIN SUCCESS SCENARIO
| Step | Action                |
| ---- | --------------------- |
| S    | 사용자가 채팅방에 접속한다.       |
| 1    | 시스템은 읽지 않은 메시지를 확인한다. |
| 2    | 메시지를 읽음 상태로 변경한다.     |
#### EXTENSION SCENARIOS
| Step | Branching Action              |
|------|-------------------------------|
| 2a   | 네트워크 오류 발생 시 읽음 상태가 변경되지 않는다. |
#### RELATED INFORMATION
- **Performance**:
  1초 이내에 완료되어야 한다.
- **Frequency**:
  매우 높은 빈도로 사용된다.
- **Concurrency**:
  다수의 사용자가 동시에 메시지를 확인하더라도 각 메시지의 읽음 상태는 정확하게 반영되어야 한다.
- **Due Date**:
  ...

### **Use case #58 : 텍스트 메시지 삭제
GENERAL CHARACTERISTICS
- **Summary**  
  사용자가 자신이 보낸 메시지를 삭제한다.
- **Scope**
  아이루트
- **Level**   
  User level
...
- **Trigger**
  사용자가 메시지 삭제를 선택한 경우
- **Success Post Condition**
    1. 메시지가 삭제되거나 삭제 표시로 변경된다.
- **Failed Post Condition**
    1. 메시지가 삭제되지 않고 오류 메시지를 출력한다.
#### MAIN SUCCESS SCENARIO
| Step | Action            |
| ---- | ----------------- |
| S    | 사용자가 채팅방에 접속한다.   |
...
- **Frequency**:
  낮은 빈도로 사용된다.
- **Concurrency**:
  여러 사용자가 동시에 동일 메시지에 접근하더라도 삭제 여부와 표시 상태가 일관되게 유지되어야 한다.
- **Due Date**:
  ...
...
classDiagram
    ...
    BoardService --> UserRepository

    PostService --> PostRepository
    PostService --> BoardRepository
    ...
    ReactionService --> UserRepository
    ReactionService --> PostRepository

    FavoriteService --> FavoriteRepository
    FavoriteService --> BoardRepository
    FavoriteService --> UserRepository

    AttachmentService --> AttachmentRepository
    AttachmentService --> PostRepository

    TagService --> TagRepository
    TagService --> PostRepository
    TagService --> BoardRepository
