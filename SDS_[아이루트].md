...
- **Success Post Condition**
  게시판 목록(자유, 공지, 문의는 상단에 고정하고 나머지 게시판은 최신순으로 나열)이 화면에 정상적으로 표시된다.
- **Failed Post Condition**
  서버 통신 실패 시 게시판 목록이 표시되지 않고 오류 메시지가 출력된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
...
- **Success Post Condition**
  새로운 게시판/게시글이 생성되고 목록에 반영된다.
- **Failed Post Condition**
  생성에 실패하며 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
...
- **Success Post Condition**
  게시판/게시글 정보가 수정되고 변경 사항이 게시판 목록에 반영된다.
- **Failed Post Condition**
  수정에 실패하며 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
...
- **Success Post Condition**
  게시판/게시글 정보가 삭제되고 변경 사항이 게시판 목록에 반영된다.
- **Failed Post Condition**
  게시판 삭제에 실패하며 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
...
- **Success Post Condition**
  검색 조건에 맞는 게시글/게시판 목록이 화면에 표시된다.
- **Failed Post Condition**
  검색 요청 처리에 실패하면 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
...
- **Success Post Condition**
  해당 게시판이 즐겨찾기 목록에 추가되거나 제거된다.
- **Failed Post Condition**
  요청 처리에 실패하면 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
...
- **Trigger**
  사용자가 특정 게시판을 클릭한다.
- **Success Post Condition**
  해당 게시판의 게시글 목록이 화면에 표시된다.
- **Failed Post Condition**
  게시글 목록 조회 실패하면 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
...
- **Trigger**
  사용자가 게시글을 클릭한다.
- **Success Post Condition**
  게시글의 상세 내용이 화면에 표시된다.
- **Failed Post Condition**
  게시글 조회 실패하면 오류 메시지가 사용자에게 표시된다.
#### MAIN SUCCESS SCENARIO
| Step | Action                                               |
| ---- | --------------------------------------------------------- |
...
classDiagram
    ...
    class User {
        ...
        +String name
        +String phoneNumber
        +String role  // Parent, Instructor, Admin
        +Boolean isDeleted
        +Date createdAt
        +Date updatedAt
    ...
    class AuthService {
        ...
        +socialLogin(providerToken)
        +logout(userId)
        +findPassword(email)
        +changePassword(userId, newPassword)
        +reissueToken(refreshToken)
        +selectUserType(userId, role)
    }
    ...
    class ProfileService {
        +getMyProfile(userId)
        +editMyProfile(userId, profileDto)
    }
    ...
    %% 관계
    Guest --> AuthService : uses (login / sign-up)
    AuthService --> User : manages authentication
    AuthService --> Token : issues
    AuthService --> System : communicates with
    ProfileService --> User : retrieves / updates
    User --> AuthService : requests auth
